    ...
    public function corporateSettlement()
    {
        ...
        try{
            ...
            foreach ($admins as $admin) {
                ...
                $last_Settlement_date = isset($logs)? $logs->settlement_date: null;
                ...
                switch ($admin->settlement_type) {
                    case 1: // weekly
                        if (!$lastSettlement || $lastSettlement->diffInDays($today) >= 7) {
                            $shouldSettle = true;
                        }
                        break;

                    case 2: // biweekly
                        if (!$lastSettlement || $lastSettlement->diffInDays($today) >= 14) {
                            $shouldSettle = true;
                        }
                        break;
