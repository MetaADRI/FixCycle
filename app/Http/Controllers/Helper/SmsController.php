...
//                        return $response;
    ...
    public function SendWhatsAppSms($payload){
       ...
       $string_file = $this->getStringFile($merchant_id);
        $message = $otp;
        $response = "No response found";
        if (!empty($phone) && !empty($config->provider_slug)) {
            switch ($config->provider_slug) {
                case "WHATSAPP_OTP_INFOBIP":
                    $infobip = new WhatsappOtpController();
                    $response = $infobip->InfobipTemplate($phone, $message, $config->api_secret,$config->auth_token,$config->api_key,$config->additional_req,$config->sender);
                    break;
            }
        }
