    ...
    public function Otp(Request $request)
    {
        ...
        if($request->for == 'EMAIL'){
            ...
            if(isset($merchant->Configuration->whatsapp_otp_enable) && $merchant->Configuration->whatsapp_otp_enable == 1){
                ...
                $integration->proceedThirdPartyIntegrations('WHATSAPP_OTP_INFOBIP', [
                    'request' => $data
                ]);
            }else{
...
//            $radius = $distance_unit == 2 ? 3958.756 : 6367;
    ...
    public function driverBasicDetails(Request $request)
    {
        ...
        $thisWeekEarnings = $thisWeekTx->sum('sub_total_before_discount')
            + round($thisWeekTx->sum('rounded_amount'), 2)
            - $thisWeekTx->sum('company_earning')
            + $thisWeekTx->sum('cancellation_charge_applied');

        // This month
        $thisMonthTx = $earningsQuery($startOfMonth, $now);
        $thisMonthEarnings = $thisMonthTx->sum('sub_total_before_discount')
            + round($thisMonthTx->sum('rounded_amount'), 2)
            - $thisMonthTx->sum('company_earning')
            + $thisMonthTx->sum('cancellation_charge_applied');

        // All time
        $allTimeTx = $earningsQuery();
        $allTimeEarnings = $allTimeTx->sum('sub_total_before_discount')
            + round($allTimeTx->sum('rounded_amount'), 2)
            - $allTimeTx->sum('company_earning')
            + $allTimeTx->sum('cancellation_charge_applied');
