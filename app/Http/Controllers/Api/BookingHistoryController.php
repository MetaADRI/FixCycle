...
//                "value_text" => $value['PaymentMethod']['payment_method'],
    ...
    public function ActiveBookings(Request $request)
    {
        ...
        switch ((int)$request->type == 1) {
            case 1:
                $bookings = Booking::select('id', 'merchant_booking_id', 'booking_status', 'booking_timestamp', 'payment_status', 'booking_closure',  \DB::raw("CONCAT(drivers.first_name, ' ', drivers.last_name) AS driver_name"), 'pickup_location', 'drop_location')
            ...
            default:
                $bookings = Booking::select('bookings.id', 'merchant_booking_id', 'booking_status', 'booking_timestamp', 'payment_status', 'booking_closure', \DB::raw("CONCAT(drivers.first_name, ' ', drivers.last_name) AS driver_name"), 'pickup_location', 'drop_location')
                    ->join("drivers", "bookings.driver_id", "=", "drivers.id" )
                    ->whereIn('booking_status', array(1002, 1003, 1004, 1005))
                    ->where([['user_id', '=', $user_id], ['payment_status', '=', 0]])
                    ->orWhere([['booking_status', '=', 1001], ['booking_type', '=', 1], ['user_id', '=', $user_id]])
