        ...
        try {
           ...
           if($driver->Merchant->Configuration->subscription_package_type == 3){
            ...
            $earningsQuery = function ($from = null, $to = null) use ($driver_id) {
                $query = Booking::where('driver_id', $driver_id)
                    ->where('booking_status', 1005) // completed trips
            ...
            $end_of_day = $now->copy()->endOfMonth();
            $today_earned = $earningsQuery($start_of_day, $end_of_day);
            $allTimeEarnings = $today_earned->sum('sub_total_before_discount') + round($today_earned->sum('rounded_amount'), 2) - $today_earned->sum('company_earning') + $today_earned->sum('cancellation_charge_applied');
