...
export default function FoodChatPage(): React.ReactNode {
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-[430px] flex-col bg-[var(--fc-surface)]">
      <TopHeader
        title={t('food.chatTitle', runtime.locale)}
        leading={
      ...
      >
        {flow.chatMessages.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-10 text-center">
            <img src="/assets/phase-7/chat-empty.svg" alt="" className="h-40 w-60" />
            <p className="text-sm text-[var(--fc-text-secondary)]">{t('food.chatPlaceholder', runtime.locale)}</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <p className={`mt-0.5 text-[10px] ${mine ? 'text-white/70' : 'text-[var(--fc-text-secondary)]'}`}>
                    {msg.sender === 'BUSINESS_SEGMENT'
                      ? flow.chatUserName || t('food.chatTitle', runtime.locale)
                      : t('food.chatPlaceholder', runtime.locale)}
                  </p>
                </div>
              </div>
            );
      ...
      <div className="flex items-center gap-2 border-t border-[var(--fc-border)] bg-[var(--fc-surface)] px-4 py-3">
        <input
          ...
          placeholder={t('food.chatPlaceholder', runtime.locale)}
