...
import type { FoodCartItem } from '@fixcycle/api-client';

export default function FoodCartPage(): React.ReactNode {
  ...
  const handleCheckout = () => {
    router.push(`/food/checkout?storeId=${encodeURIComponent(storeId)}`);
  };
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-[430px] sm:max-w-[640px] md:max-w-[768px] lg:max-w-[1024px] md:max-w-[640px] lg:max-w-[768px] xl:max-w-[1024px] flex-col bg-[var(--fc-surface)]">
      <TopHeader
        title={t('food.cart', runtime.locale)}
        leading={
      ...
      <main className="flex-1 pb-28 pt-3">
        {loading && cart.products.length === 0 ? (
          ...
          <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
            ...
            <p className="text-base font-bold text-[var(--fc-text-primary)]">{t('food.cartEmpty', runtime.locale)}</p>
            <p className="max-w-xs text-sm text-[var(--fc-text-secondary)]">
              {t('food.cartEmptyMessage', runtime.locale)}
            </p>
          ...
          <>
            ...
            <div className="px-4 pt-4">
              <Button block variant="primary" disabled={checkoutDisabled} onClick={handleCheckout}>
                {t('food.checkout', runtime.locale)}
              </Button>
            </div>
          </>
        )}
...
function CartItemRow({
  ...
  return (
    <li className="flex items-center gap-3 rounded-2xl border border-[var(--fc-border)] bg-[var(--fc-surface-raised)] p-3">
      <img src="/assets/phase-7/food-placeholder.svg" alt="" className="h-11 w-11 shrink-0 rounded-xl" />
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex items-center gap-3">
          ...
          <button
            ...
            aria-label={t('food.cartEmpty', runtime.locale)}
