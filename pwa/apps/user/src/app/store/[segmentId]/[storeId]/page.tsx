...
export default function StoreDetailPage(): React.ReactNode {
  ...
  const visibleProducts = useMemo(() => {
    if (flow.searchingOpen) return flow.searchResult?.products ?? products;
    if (!flow.selectedCategory) return products;
    return products.filter((p) => p.category_id === flow.selectedCategory?.id);
  }, [products, flow.selectedCategory, flow.searchingOpen, flow.searchResult]);

  const openProductPage = useCallback(
    (p: StoreProduct) => {
      router.push(`/store/${segmentId}/${storeId}/product/${p.id}`);
    },
    [router, segmentId, storeId],
  );

  const openCartPage = useCallback(() => router.push(`/store/${segmentId}/cart`), [router, segmentId]);
  const openChatPage = useCallback(
    () => router.push(`/store/${segmentId}/${storeId}/chat`),
    [router, segmentId, storeId],
  );

  const handleAdd = async (p: StoreProduct) => {
    if (p.variants.length > 1 || p.options.length > 0) {
      openProductPage(p);
      return;
    }
    await flow.addToCartSimple(p);
    setAdded(p.id);
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-[430px] sm:max-w-[640px] md:max-w-[768px] lg:max-w-[1024px] md:max-w-[640px] lg:max-w-[768px] xl:max-w-[1024px] flex-col bg-[var(--fc-surface)]">
      <TopHeader
        ...
        trailing={
          <button type="button" aria-label={t('store.chat', runtime.locale)} onClick={openChatPage}>
      ...
      <main className="flex-1 pb-28 pt-3">
        <section className="px-4 pb-3">
          <img
            src={flow.isPharmacy ? '/assets/phase-8/pharmacy-banner.svg' : '/assets/phase-8/store-banner.svg'}
            alt=""
          ...
          {store.is_open !== 1 ? (
            <div className="mt-3 flex items-center gap-2 rounded-xl bg-[var(--fc-text-secondary)]/10 px-3 py-2 text-xs font-semibold text-[var(--fc-text-secondary)]">
              ...
              {t('store.todayClosed', runtime.locale)}
            </div>
          ) : null}

          {/* Search */}
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-[var(--fc-border)] bg-[var(--fc-surface-raised)] px-3 py-2">
            <Icon name="search" size={16} className="text-[var(--fc-text-secondary)]" />
            <input
              value={flow.searchQuery}
              onChange={(e) => {
                flow.setSearchQuery(e.target.value);
                if (e.target.value.trim().length > 0 && !flow.searchingOpen) flow.openSearch();
                if (e.target.value.trim() === '') flow.closeSearch();
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') void flow.runSearch(flow.searchQuery);
              }}
              placeholder={t('store.searchPlaceholder', runtime.locale)}
              className="min-w-0 flex-1 bg-transparent text-sm text-[var(--fc-text-primary)] outline-none placeholder:text-[var(--fc-text-secondary)]"
            />
            {flow.searchingOpen ? (
              <button type="button" aria-label={t('store.search', runtime.locale)} onClick={() => void flow.runSearch(flow.searchQuery)}>
                <Icon name="close" size={16} className="text-[var(--fc-bg-secondary)]" />
              </button>
            ) : null}
          </div>
        </section>
        ...
        {!flow.searchingOpen ? (
          categories.length > 1 ? (
            <nav
              aria-label={t('store.categories', runtime.locale)}
        ...
        {visibleProducts.length === 0 ? (
          <p className="px-4 py-10 text-center text-sm text-[var(--fc-text-secondary)]">
            {flow.searchingOpen ? t('store.noResults', runtime.locale) : t('store.errMenu', runtime.locale)}
          </p>
        ) : (
            ...
            {visibleProducts.map((p) => (
              ...
              >
                <img
                  src={
                    flow.isPharmacy
                      ? '/assets/phase-8/pharmacy-product.svg'
                      : '/assets/phase-8/product-placeholder.svg'
                  }
                  alt=""
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-[var(--fc-text-secondary)]">
                    {p.weight || `${p.variants.length} ${t('store.variant', runtime.locale).toLowerCase()}`}
                  </span>
                  {p.description ? (
                ...
                <button
                  ...
                  aria-label={t('store.add', runtime.locale)}
